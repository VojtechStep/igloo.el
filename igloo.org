...
- Prefer lazy loading
- Use =igloo-use-package= for builtin pakages too
- Follow this keyword order for =use-package= clauses:
...
'(:disabled
  ...
  :config)
#+end_src
- Declare keybindings to package commands in its =use-package= declaration, via =:general=
