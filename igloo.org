...
(igloo-use-package consult
  ...
  (igloo-local-leader-with-minor outline
    "g" '(consult-outline :wk "go to heading" :jump t))
  :preface
...
#+begin_src emacs-lisp :noweb-ref eshell-prompt :tangle no
(eval-when-compile
  (defmacro igloo--eshell-segment (form fg &optional bg &rest props)
    (declare (indent defun))
    (unless bg (setq bg ''term-color-black))
    (list 'propertize
          form
          ''face
          (append
...
(defun igloo--eshell-prompt ()
  (concat
   ...
   (igloo--eshell-segment "@"
     'term-color-white)
   (igloo--eshell-segment (system-name)
     'term-color-white)
   " "
   (igloo--eshell-segment
...
(igloo-use-package fish-mode
  ...
  :init
  (igloo-link-offset 'fish-indent-offset))
#+end_src
...
(igloo-use-package cc-mode
  ...
  :apheleia
  (clang-format . ("clang-format" "--assume-filename" file))
...
(igloo-use-package mu4e
  ...
  (declare-function mu4e-message "mu4e-utils")
...
(igloo-use-package org
  ...
  (igloo-local-leader org-mode-map
    "g" '(consult-org-heading :wk "go to heading" :jump t))
  :preface
